...
> **Why this page is structured this way:** IGRCs operate in the gap between (a) <abbr title="SEBI Complaints Redress System">SCORES</abbr> first-level resolution by the broker and (b) full <abbr title="Online Dispute Resolution">ODR</abbr> arbitration. They handle fee-amount disputes within an exchange's quasi-judicial framework. The page explains the chain — who sits on the committee, how a matter reaches it, what they decide, and how a broker contests or complies.

## TL;DR

- The <abbr title="Investor Grievance Redressal Committee">IGRC</abbr> is a stock-exchange / depository statutory committee under the <abbr title="Securities and Exchange Board of India">SEBI</abbr> (Stock Exchanges and Clearing Corporations) Regulations 2018 and the SEBI Master Circular for Stock Exchanges & Clearing Corporations `SEBI/HO/MRD-PoD2/CIR/P/2024/00181`.
- IGRCs operate at <abbr title="National Stock Exchange of India">NSE</abbr>, <abbr title="BSE Limited (formerly Bombay Stock Exchange)">BSE</abbr>, <abbr title="Multi Commodity Exchange of India">MCX</abbr> (for commodity derivatives), NSE Clearing, <abbr title="Indian Clearing Corporation Limited">ICCL</abbr>, <abbr title="Multi Commodity Exchange Clearing Corporation Limited">MCXCCL</abbr>, plus <abbr title="Central Depository Services (India) Limited">CDSL</abbr> and <abbr title="National Securities Depository Limited">NSDL</abbr> on the depository side. Most broker-related disputes flow through NSE / BSE IGRC; depository-side issues through CDSL / NSDL IGRC.
...
- Workflow: SCORES first-level resolution → broker <abbr title="Action Taken Report.">ATR</abbr> → Investor First Review → Designated Body (here, the exchange) routes monetary disputes to IGRC → IGRC hearing → IGRC order → broker compliance OR escalation to arbitration (ODR).
...
<Aside type="caution">
**IGRC is not the only exchange-level investor service body.** Each exchange also operates an **Investor Service Cell (<abbr title="Investor Service Centre.">ISC</abbr>)** that handles administrative complaint intake, claim verification, and <abbr title="Investor Protection Fund">IPF</abbr> claims (under `SEBI/HO/MRD-PoD2/CIR/P/2024/00181` IPF chapter). ISC is staff-administered. IGRC is committee-administered (committee of PIDs + experts + investor reps). For most operational purposes, the ISC is the broker's day-to-day interface; IGRC enters the picture only on substantive disputes.
</Aside>
...
**Mandatory members (per SEBI Master Circular for SE & <abbr title="Clearing Corporation (NCL, ICCL, MCXCCL — context-dependent).">CC</abbr> `SEBI/HO/MRD-PoD2/CIR/P/2024/00181`):**
...
- CDSL / NSDL — depository-level IGRC for <abbr title="Depository Participant">DP</abbr>-investor disputes
...
- Verify the investor is a client of the named broker (<abbr title="Unique Client Code">UCC</abbr> check)
...
- Trade extracts, contract notes, <abbr title="Know Your Customer (process).">KYC</abbr> artefacts (pulled by ISC from broker via formal request)
...
- **Documentary pack** — KYC artefacts, contract notes, trade logs, communications, <abbr title="Authorized Person">AP</abbr> records, surveillance evidence
...
Orders are signed by the Chair and circulated to investor, member, exchange Member Compliance team, and SEBI <abbr title="Office of Investor Assistance and Education (SEBI)">OIAE</abbr> (if SCORES-routed).
...
- Account opening file (KYC artefacts, <abbr title="KYC Registration Agency">KRA</abbr> / <abbr title="Central KYC (records registry)">CKYC</abbr> trail)
...
- **<abbr title="Margin Trading Facility">MTF</abbr> / margin trading dispute** — interest charges, liquidation timing, collateral valuation
...
<abbr title="Non-Resident Indian">NRI</abbr> complaints follow normal IGRC pathway. Communication is by email/video. <abbr title="Foreign Exchange Management Act 1999">FEMA</abbr>-compliance issues are tagged for <abbr title="Reserve Bank of India">RBI</abbr> co-ordination.
...
- <abbr title="Asset Management Company (mutual funds context) / Annual Maintenance Charges (depository context).">AMC</abbr> / DP charges
- KYC / nominee modification issues
- <abbr title="Beneficial Owner">BO</abbr> master corrections
