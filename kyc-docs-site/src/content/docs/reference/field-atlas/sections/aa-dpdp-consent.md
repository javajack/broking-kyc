...
> **Why this page is structured this way:** This is the field-first view for Section AA: DPDP Act 2023 Consent Management. Each row is one field-destination relationship. To see the same data from the destination's perspective, jump from the [atlas overview](/broking-kyc/reference/field-atlas/).
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AA-data_retention_end_date | Data Retention End Date | back-office | data_retention_end | DATE YYYYMMDD | on-modify | derived from Y | 8 yrs per SEBI Stock Brokers Regulations; auto-purge after | SEBI Stock Brokers Regulations 2026 |
| AA-data_retention_end_date | Data Retention End Date | regulatory-reports | RetentionEndDate | DATE YYYYMMDD | on-modify | derived from Y | 8-year horizon per SEBI Stock Brokers Regulations; aligned to ECN archive retention | SEBI/HO/MIRSD/POD-1/P/CIR/2025/94 |
...
| AA-dpdp_consent_withdrawal_date | DPDP Withdrawal Date | aml-fiu | DPDP_OVERRIDE_FLAG | DATE YYYYMMDD | on-event | [direct] | PMLA obligations override DPDP withdrawal; STR/CTR continue even after consent withdrawn | SEBI/HO/MIRSD/SECFATF/P/CIR/2024/78 |
...
| AA-dpdp_third_party_sharing_consent | DPDP Third-Party Sharing Consent | regulatory-reports | none | none | on-modify | [direct] | gates whether broker can share UCC-keyed margin file rows with vendor risk-platforms; consent withdrawal stops downstream flows | [industry typical] |
