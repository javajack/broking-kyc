...
- **The Integration <abbr title="Directed Acyclic Graph.">DAG</abbr>** — your position in the dependency graph; what blocks what.
...
2. **[Field Atlas Overview](/broking-kyc/reference/field-atlas/)** — if your product produces or consumes data, find the per-destination page that maps to your product. For example:
   - <abbr title="Know Your Customer (process).">KYC</abbr> verification → [KYC verification destination](/broking-kyc/reference/field-atlas/destinations/kra/) and [<abbr title="Central KYC (records registry)">CKYC</abbr>](/broking-kyc/reference/field-atlas/destinations/ckyc/).
   ...
   - <abbr title="Account Aggregator (RBI-licensed NBFC-AA)">AA</abbr> / consent manager → most relevant in [Income Verification](/broking-kyc/operations/compliance-blueprint/#kyc-lifecycle-41-entries) and bank verification rows.
   - DLT / <abbr title="Short Message Service.">SMS</abbr> → [DLT comms destination](/broking-kyc/reference/field-atlas/destinations/dlt-comms/).
   - <abbr title="Order Management System">OMS</abbr> / <abbr title="Risk Management System">RMS</abbr> / back-office → [back-office destination](/broking-kyc/reference/field-atlas/destinations/back-office/), [RMS destination](/broking-kyc/reference/field-atlas/destinations/rms/).
   - Surveillance / <abbr title="Anti-Money Laundering">AML</abbr> → [AML/<abbr title="Financial Intelligence Unit">FIU</abbr> destination](/broking-kyc/reference/field-atlas/destinations/aml-fiu/).

3. **[Integration DAG Overview](/broking-kyc/operations/integration-dag/)** — find your phase. Your product likely sits in one of: Onboarding (KYC verification, eSign, AA, penny drop) / <abbr title="Beginning Of Day">BOD</abbr> (file fetch from exchanges) / Trading Hours (OMS, RMS, surveillance) / <abbr title="End Of Day">EOD</abbr> & Settlement (back-office, DLT comms) / Lifecycle Events (modification routing).
...
4. **Deep dive for your domain:**
   ...
   - **OMS / RMS** → [OMS Internals deep-dive](/broking-kyc/deep-dives/trading-day/oms-internals/) + [RMS / <abbr title="Standard Portfolio Analysis of Risk">SPAN</abbr> Methodology](/broking-kyc/deep-dives/trading-day/rms-span-methodology/).
   ...
   - **Surveillance** → [Surveillance — NORMS / <abbr title="Graded Surveillance Measure">GSM</abbr> / <abbr title="Additional Surveillance Measure">ASM</abbr> deep-dive](/broking-kyc/deep-dives/trading-day/surveillance-norms-gsm-asm/).
...
- **Does my product need to be empanelled with the exchange?** → Depends on category. <abbr title="Computer-to-Computer Link.">CTCL</abbr> / algo vendors have <abbr title="National Stock Exchange of India">NSE</abbr> / <abbr title="BSE Limited (formerly Bombay Stock Exchange)">BSE</abbr> / <abbr title="Multi Commodity Exchange of India">MCX</abbr> empanelment paths. See [OMS Internals deep-dive](/broking-kyc/deep-dives/trading-day/oms-internals/) + [Retail Algo Framework](/broking-kyc/deep-dives/trading-day/retail-algo-framework/).
...
- **What's the broker's <abbr title="Cybersecurity and Cyber Resilience Framework">CSCRF</abbr> expectation for me as a vendor?** → [CSCRF deep-dive — vendor due diligence section](/broking-kyc/deep-dives/compliance-audit/cscrf-deep-dive/).
