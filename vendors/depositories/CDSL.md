...
**Integration note**: CVL KRA is a separate integration (see V4 in VENDOR_INTEGRATIONS.md). This document covers only the depository (CDSL) integration. However, we use CVL KRA status as a prerequisite for BO account opening.
...
> **IMPORTANT**: The pre-allocation mechanism must be confirmed with CDSL during DP registration. The exact process (CDAS module vs email request vs API) varies and is communicated during onboarding. The Line 01 file format field at Position 3 (Client ID) supports both modes — auto-assigned (left blank) or pre-assigned (populated by DP).

### 5.5 Storage in Our System

Per KYC_MASTER_DATASET.md Section H:
...
**Our Flow**: KRA pipeline and CDSL pipeline run in parallel after checker approval (see kyc-flow.md Section 8). However, if KRA status is not yet Registered at the time of CDSL API submission, the CDSL API will reject with `BO_ERR_020`.

**Mitigation**: Our batch pipeline checks KRA status before submitting to CDSL. If KRA is still under process, CDSL submission is deferred and retried (see retry logic in kyc-flow.md Section 9).
